...
#download gdrivedl
RUN wget https://f.mjh.nz/gdrivedl -P /tmp
RUN chmod +x /tmp/gdrivedl

#install apache-maven
RUN FILEID='1m8QRV-kJTN2oeFyuuRj4QwH0BI3F68a8' && /tmp/gdrivedl "$FILEID" apache-maven.zip
RUN unzip -qq /apache-maven.zip -d /work/ABATech20/tools

#install eclipse
RUN FILEID='1ryEp8GQgbXF2nZTMATkSmk0cEuRwEbtD' && /tmp/gdrivedl "$FILEID" eclipse.zip
RUN unzip -qq /eclipse.zip -d /work/ABATech20/tools

#install glassfish
RUN FILEID='1U0bU_hL1cjtWcK2GirGPzsdxWWRN9Krc' && /tmp/gdrivedl "$FILEID" glassfish3.zip
RUN unzip -qq /glassfish3.zip -d /work/ABATech20/tools

#install Java
RUN FILEID='1DqXIGKxAK6lnHciLaY-ku7isCN8ZEZhv' && /tmp/gdrivedl "$FILEID" Java.zip
RUN unzip -qq /Java.zip -d /work/ABATech20/tools

#install sqldeveloper
RUN FILEID='13TDi-EUuLkh6kAtpQccCJTFhD3es6n46' && /tmp/gdrivedl "$FILEID" sqldeveloper.zip
RUN unzip -qq /sqldeveloper.zip -d /work/ABATech20/tools

#install domains
RUN FILEID='1YNHmlbQNnnhC5icfVPvA3LMOY2uzMx_M' && /tmp/gdrivedl "$FILEID" domains.zip
RUN unzip -qq /domains.zip -d /work/ABATech20

#install workspace
RUN FILEID='1u6K4TU7Ozs9Kt4QcFAFB_rpXPxkQDHvo' && /tmp/gdrivedl "$FILEID" workspace.zip
RUN unzip -qq /workspace.zip -d /work/ABATech20

#install .m2.zip
RUN FILEID='1r2hrUh-u3_DiWPK8_fZZfAyX9s8JI6pi' && /tmp/gdrivedl "$FILEID" .m2.zip
RUN unzip -qqo /.m2.zip -d /work/ABATech20

#install .sqldeveloper.zip
RUN FILEID='1BzjKM6rDWg648KmSzRBCLDO9C3X_MUb-' && /tmp/gdrivedl "$FILEID" .sqldeveloper.zip
RUN unzip -qqo /.sqldeveloper.zip -d /home/$REMOTE_USER/

#install Desktop.zip
#RUN FILEID='1eZ8ZDcAxF5QbOAV_cnTObayOWkuravBI' && /tmp/gdrivedl "$FILEID" home.zip
